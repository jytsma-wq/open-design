...
describe('chat prompt helpers', () => {
  ...
  it('keeps large Design Files workspace snapshots compact before appending them to agent prompts', () => {
    ...
    const block = formatDesignFilesWorkspaceHint('/tmp/open-design-project', files, []);

    expect(block.length).toBeLessThanOrEqual(60_000);
    expect(block).toContain('Current Design Files snapshot: 0 folders, 50000 files.');
    expect(block).toContain('Files:');
    expect(block).toContain('49920 more files omitted');
