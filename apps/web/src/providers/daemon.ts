...
function transcriptEntry(message: ChatMessage): string {
  ...
  const sanitized =
    message.role === 'assistant'
      ? sanitizePriorAssistantTurnForTranscript(trimmed, persistedArtifactFilesOf(message))
      : trimmed;
...
export function buildDaemonTranscript(history: ChatMessage[], targetAgentId?: string): string {
  ...
  const transcript = buildBoundedHistoryTranscript(scopedHistory, transcriptBudget);
