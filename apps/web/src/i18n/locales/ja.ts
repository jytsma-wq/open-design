...
export const ja: Dict = {
  ...
  'settings.connectorsHelpUnsaved': '未保存の変更があります — 保存をクリックすると、このキーをローカル daemon に保存して下のカタログを有効にします。',
  ...
  'settings.orbit.emptyAria': 'Orbit はまだ実行されていません',
  'settings.orbit.emptyEyebrow': '初回実行待ち',
  'settings.orbit.emptyTitle': '日次サマリーはここに表示されます',
  'settings.orbit.emptyBodyScheduled': 'Orbit はスケジュール済みです — 次回の自動実行で live artifact とコネクターアクティビティの内訳がこのカードに公開されます。',
  'settings.orbit.emptyBodyManual': 'ヘッダーの「実行して開く」で初回実行を開始してください。コネクターの内訳と更新可能な live artifact がここに表示されます。',
